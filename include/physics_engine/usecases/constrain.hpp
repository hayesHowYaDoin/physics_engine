...
physics::domain::PositionVector2D<Length>
findClosestPoint(physics::domain::PositionVector2D<Length> const &point,
  ...
  auto u{((pointX - edgeFirstX) * dx + (pointY - edgeFirstY) * dy) /
         (dx * dx + dy * dy)};
  ...
  return physics::domain::PositionVector2D<Length>{edgeFirstX + u * dx,
                                                   edgeFirstY + u * dy};
}
...
Particle<Units> resolveConstraint(
  ...
  for (auto const &edge : constraint.getEdges()) {
    ...
    if (distance < radius) {
      ...
      auto correction{
          physics::domain::Vector2D::fromPolar(normalAngle, radius - distance)};
      particleCopy.position = particleCopy.position + correction;

      particleCopy.velocity =
          physics::domain::reflect(particleCopy.velocity, normalAngle);
    }
  }
