...
struct Vector2D {
  ...
  public:
    ...
    [[nodiscard]] constexpr auto getAngle() const noexcept {
      return RetType(atan2(m_y, m_x));
    }

    template <physics::units::IsMagnitudeUnit RetType>
    [[nodiscard]] constexpr auto getMagnitude() const noexcept {
    ...
    auto getElements() const {
      return std::make_tuple(physics::detail::Element{"x", m_x},
  ...
  template <physics::units::IsMagnitudeUnit MagnitudeType>
  [[nodiscard]] static constexpr Impl<MagnitudeType>
  fromComponents(MagnitudeType x, MagnitudeType y) noexcept {
    return Impl<MagnitudeType>{std::move(x), std::move(y)};
  }

  template <physics::units::IsMagnitudeUnit MagnitudeType,
            physics::units::IsAngleUnit AngleType>
  [[nodiscard]] static Impl<MagnitudeType>
  fromPolar(AngleType angle, MagnitudeType magnitude) noexcept {
    ...
    if (magnitude.template to<float>() < 0.0f) {
      ...
      angleRadians += physics::units::angle::radians<double>(std::numbers::pi);
    }
  ...
  template <physics::units::IsMagnitudeUnit MagnitudeType>
  [[nodiscard]] static constexpr bool compare(Impl<MagnitudeType> const &lhs,
                                              Impl<MagnitudeType> const &rhs,
                                              double epsilon) noexcept {
    auto xDiff{physics::units::fabs(lhs.template getX<MagnitudeType>() -
                                    rhs.template getX<MagnitudeType>())};
    auto yDiff{physics::units::fabs(lhs.template getY<MagnitudeType>() -
                                    rhs.template getY<MagnitudeType>())};
...
template <physics::units::IsMagnitudeUnit RhsType,
          physics::units::IsMagnitudeUnit LhsType>
[[nodiscard]] constexpr auto
operator+(Vector2D::Impl<LhsType> const &lhs,
...
[[nodiscard]] constexpr auto operator*(Vector2D::Impl<LhsType> const &lhs,
...
template <physics::units::IsMagnitudeUnit RhsType>
[[nodiscard]] constexpr auto Vector2D::Impl<LhsType>::dot(
    Vector2D::Impl<RhsType> const &rhs) const noexcept {
...
template <physics::units::IsMagnitudeUnit LhsType>
template <physics::units::IsMagnitudeUnit RhsType>
[[nodiscard]] constexpr auto Vector2D::Impl<LhsType>::cross(
    Vector2D::Impl<RhsType> const &rhs) const noexcept {
